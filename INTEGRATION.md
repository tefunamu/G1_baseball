...
### 💡【案】G1の `/pitch` 発火 実装案（SDK調査ベース・要チーム合意 2026-06-30）
> これは調査に基づく**提案（たたき台）**です。最終的な実装方針はロボ担当と相談して決めてください。

**調査結論：G1 SDKに「モーション完了の通知機能」は無い。でも（この案なら）要らない。**
自前で投球モーションを組むなら、制御スクリプトが"ボールを離す時刻"を知っているので、
その瞬間に `/pitch` を叩くのが一番シンプルで正確（ミリ秒単位）、というのが本案の骨子。
